...
{
    unsigned l = strlen(s); // 求s长度
    unsigned v = 0;
    unsigned h = (l > 2 && s[0] == '0' && (s[1] == 'x' || s[1] == 'X'));
    unsigned char c;
    unsigned char t;
    if (!h) return atoi(s); // s转成字符串整型
    if (l > 10) return 0; //　字符长度超过10, 退出程序
    for (s += 2; c = *s; s++)
    {
        if (c >= 'A' && c <= 'F') c += 32;
        if (c >= '0' && c <= '9') t = c - '0';
...
void generate_frame(VCI_CAN_OBJ *can)
{
    ...
    can->DataLen = 1 + (rand() % 8); // random data length: 1~8, %求余
    unsigned i;
    for (i = 0; i < can->DataLen; i++)
    {
        can->Data[i] = rand() & 0xff; // random data, & 位与运算
        can->ID ^= can->Data[i]; // id: bit0~7, checksum of data0~N, ^= 异或运算
    }
